...
import com.github.vincemann.springrapid.acl.proxy.ConfigureProxies;
...
import com.github.vincemann.springrapid.acl.proxy.SecurityServiceExtensionProxyBuilder;
...
public class AnnotationCrudServiceSecurityProxyFactory {
        ...
        if(unwrappedBean instanceof CrudService){
            ...
            for (Proxy proxy : proxies) {
                ...
                CrudService proxyBean = new ServiceExtensionProxyBuilder<>(proxiedBean)
                        .addServiceExtensions(resolveExtensions(proxy.plugins()).toArray(new AbstractServiceExtension[0])).build();
