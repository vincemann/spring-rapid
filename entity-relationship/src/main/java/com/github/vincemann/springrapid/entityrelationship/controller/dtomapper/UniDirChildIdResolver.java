...
public class UniDirChildIdResolver extends EntityIdResolver<UniDirChild, UniDirChildDto> {
    ...
    public void resolveEntityIds(UniDirChild mappedUniDirChild, UniDirChildDto uniDirChildDto) throws BadEntityException, EntityNotFoundException {
        ...
        Map<Class<UniDirParent>, Collection<Serializable>> parentTypeIdCollectionMappings = uniDirChildDto.findUniDirParentIdCollections();
        for (Map.Entry<Class<UniDirParent>, Collection<Serializable>> entry : parentTypeIdCollectionMappings.entrySet()) {
            Collection<Serializable> idCollection = entry.getValue();
            for (Serializable id : idCollection) {
                Class entityClass = entry.getKey();
                UniDirParent parent = findEntityFromService((Class<IdentifiableEntity>) entityClass, id);
                //set parent of mapped child
                mappedUniDirChild.linkUniDirParent(parent);
                //dont set backref, because Parent does not know about Child (uniDir)
    ...
    public void resolveDtoIds(UniDirChildDto mappedDto, UniDirChild serviceEntity) {
        ...
        for (Collection<UniDirParent> parentCollection : serviceEntity.findUniDirParentCollections().keySet()) {
            for (UniDirParent parent : parentCollection) {
                mappedDto.addUniDirParentId(parent);
            }
        }
    }
}
